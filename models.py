...
entry_tag = db.Table(
    ...
    db.Column(
        "entry_id",
        db.Integer,
        db.ForeignKey("entries.id", ondelete="CASCADE", onupdate="CASCADE"),
        primary_key=True,
    ),
    db.Column(
        "tag_id",
...
class Tag(db.Model):
    ...
    type = db.Column(Enum("entry", "goal", name="tag_types"), nullable=True)
    category = db.Column(
        db.Integer, nullable=True
    )  # Optional, to use for showing/hiding layers of tags


def init_db(app):
    print("Wiping database and initializing new data!")
    user_1 = User(
        oauth_id="test_oauth_id", birth=date.fromisoformat("1995-03-06"), exp_years=80
    )
    tag_1 = Tag(user_id=1, name="Tag 1", color="Color 1")
    tag_2 = Tag(user_id=1, name="Tag 2", color="Color 2")
    entry_1 = Entry(
        user_id=1,
        start=date.fromisoformat("2023-09-04"),
        tags=[tag_1],
        note="Content of entry with tag 1",
    )
    entry_2 = Entry(
        user_id=1,
        start=date.fromisoformat("2023-09-11"),
        tags=[tag_1, tag_2],
